...
//
define(["./buildControl", "./packageJob", "bdParse"], function(buildControl, packageJob, parser) {
var 
  ...
  if (resource.tree) {
    ...
    try {
      ...
      for (var dep in depsSet) {
        resolvedDeps= resolvedDeps.concat(resolveModuleId(dep, thisObject));
      }
      resource.deps= resolvedDeps;
    } catch (e) {
