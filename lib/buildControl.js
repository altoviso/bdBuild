define(["./argv", "./fileUtils", "./defaultBuildControl", "./stringify", "loader"], function(args, fileUtils, defaultBuildControl, stringify, loader) {
  ...
  var
    ...
    mixBuildControlObject= function(src) {
      ...
      //   paths, packages, packagePaths, packageMap, pluginProcs, transforms, loaderConfig, staticHasFlags, buildFlags
      // ...are mixed one level deep; all others are over-written
      for (var p in src) {
        if (!/(paths)|(packages)|(packagePaths)|(packageMap)|(pluginProcs)|(transforms)|(loaderConfig)|(staticHasFlags)|(buildFlags)/.test(p)) {
          bc[p]= src[p];
        }
      }
      ...
      ["paths","plugins","transforms","loaderConfig","staticHasFlags","buildFlags"].forEach(function(p) {
        bc[p]= mix(bc[p], src[p]);
      });
    };
