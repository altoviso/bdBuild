define(["./argv", "./fileUtils", "./defaultBuildControl", "./gates", "./stringify", "loader"], function(args, fileUtils, defaultBuildControl, gates, stringify, loader) {
  ...
  (function() {
    ...
    for (var packageName in bc.packages) {
      ...
      if (!pack.dirs && !pack.trees) {
        ...
        pack.trees= [[compactPath(catPath(pack.location, pack.lib)), compactPath(catPath(pack.destLocation, pack.destLib)), "*/.*", "*~"]];
    ...
    bc.nameToUrl= function(name, ext, referenceModule) {
      ...
      var
        match= name.match(/(.+)(\.[^\/]+)$/),
        moduleInfo= getSrcModuleInfo(match && match[1] || name, referenceModule),
        url= moduleInfo.url;
  ...
  if (bc.check) (function() {
    ...
    for (p in bc.packages) {
      ...
      packages.push({
        name:pack.name, lib:pack.lib, main:pack.main, location:pack.location,
