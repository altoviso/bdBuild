define(["./buildControl", "./fileUtils", "fs"], function(bc, fileUtils, fs) {
    ...
    processPackage= function(pack, destPack) {
      ...
      filenames.forEach(function(filename) {
        ...
        var 
          ...
          mid= filename.substring(libPathLength, maybeModule ? filename.length-3 : filename.length),
          moduleInfo= maybeModule && bc.getSrcModuleInfo(prefix+mid);
        if (!maybeModule) {
      ...
      for (var mid in pack.modules) {
        var moduleInfo= bc.getSrcModuleInfo(prefix+mid);
        if (typeof pack.modules[mid]=="string") {
          moduleInfo.url= pack.modules[mid];
        }
        maybeAmdModules[mid]= moduleInfo;
        delete notModules[mid];
      };
  ...
  return function() {
    ...
    // advise all modules if they are to be written as a layer
    for (var mid in bc.layers) {
      ...
      if (!resource) {
        ...
        resource.layerExclude= layer.exclude;
        resource.layerInclude= layer.include;
        resource.bootText= layer.bootText;
        resource.boot= layer.boot;
      }
    }
  };
});
