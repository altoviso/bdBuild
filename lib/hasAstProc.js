///
// \module bdBuild/jsResourceAstProc
//
define(["./buildControl", "bdParse"], function(buildControl, bdParse) {
...
var 
  ...
  evalHasExpr= function(location) {
    ...
    try {
      ...
      console.log(resource.srcFilename + "(" + sLine + "):failed const has eval:");
      console.log(code);
      console.log(e);      
      return false;
    }
  ...
  if (children[0].value=="(") {
    ...
    if (name.type===asnName && name.children.value=="has") {
      ...
      if (feature) {
        ...
        if (buildControl.staticHasFlags.hasOwnProperty(feature)) {
          ...
          // if feature is true
          var 
            replacement= buildControl.staticHasFlags[feature] ? "1" : "0",
          ...
          var featureVector= buildControl.hasLocations[feature]= buildControl.hasLocations[feature] || []; 
...
asnAtom.hasPpReplace=
  function() {
    console.log("trying to replace an atom in the has preprocessor");
  };
...
asnRoot.hasPpReplace=
  function(thisNode, children, original, replacement) {
    ...
    console.log("could not find original in hasPpReplace");
  };
...
return function(resource_) {
  if (resource_.tree) {
    resource= resource_;
    deleteList= resource.deleteList= resource.deleteList || [];
    text= resource.text;
    try {
