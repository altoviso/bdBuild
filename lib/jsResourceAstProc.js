...
//
define(["./buildControl", "bdParse"], function(buildControl, parser) {
...
symbols["asntConditional"].hasPP= function(children, thisNode) {
  if (hasPP(children[0])) {
    var hasExprVal= evalHasExpr(children[0].location);
    if (hasExprVal===true) {
      // delete the if statement; keep the true statement
      ...
      if (children[2]) {
        deleteList.push(new location(thisNode.location.start(), children[2].location.start(-1)));
        deleteList.push(new location(children[2].location.end(), thisNode.location.end()));
        hasPP(children[2]);
      } else {
        deleteList.push(thisNode.location);
      }
