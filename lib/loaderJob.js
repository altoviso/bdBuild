...
var
  ...
  bootLayerTemplate= 'require({cache:{"*1":function() {\n*3\n}}});\nrequire(["*2"]);\n',
    ...
    try {
      ...
      for (var mid in bc.layers) {
        ...
        if (module && layer.boot) {
          var text= bootLayerTemplate.replace(/\*1/g, module.pqn).replace(/\*2/, mid).replace(/\*3/, module.getLayerText());
          bc.waiting++;

          fileUtils.write(bc.sandbox + layer.boot, loaderText + getConfigText(config) + text, cb);
        }
      }
    } catch (e) {
      cb(e);
