...
//
define(["./fileUtils", "./buildControl", "./configStringify", "./packageJob", "bdParse", "fs", "text!bdBuild/has/bdBuildHas.js", "text!bdBuild/has/naiveHas.js"], function(fileUtils, bc, configStringify, packageJob, bdParse, fs, bdBuildHasText, naiveHasText) {
var
  read= function(
    cb
  ) {  
  ...
  getConfigText= function(config) {
    ...
    if (bc.has) {
      if (bc.has=="*bdBuild") {
        hasText= bdLoadHasText;
      } else {
        var hasFilename= packageJob.getModuleInfo(bc.has, 0).url;
        hasText= fs.readFileSync(hasFilename, "utf8");
      }
    }
    return "(\nthis.require || {},\n" + configText + ",\n" + hasText + "\n);\n";
  },
