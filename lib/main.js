...
require(args.amdLoader || config.amdLoader);
define("*config", args.amdLoaderConfig || config.amdLoaderConfig);
...
define("*load", ["bdBuild/fileUtils", "bdBuild/buildControl"], function(fileUtils, buildControl) {
  ...
  if (buildControl.check) {
    ...
    fileUtils.ensurePath(buildControl.sandbox);
    var
      procNames= [], 
      deps= [];
    ...
    define("*load", deps, function() {
      ...
      buildControl.waiting++;
      define("*load", buildControl.jobList, function() {
