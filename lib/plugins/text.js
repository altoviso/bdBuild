...
define(["bdBuild/buildControl", "../packageJob", "fs"], function(bc, packageJob, fs) {
  var
    ...
    //cacheTemplate= 'define("text!!" + require.nameToUrl("*1") + "*2", 0, *3);\n\n',
    cacheTemplate= 'require(["text"], function(text) {text.cache(require.nameToUrl("*1") + "*2", *3);});\n\n',

    writeModuleToCache= function(cache) {
      cache.freeText+= cacheTemplate.replace(/\*1/, this.path).replace(/\*2/, this.filetype).replace(/\*3/, JSON.stringify(fs.readFileSync(this.srcFilename, "utf8")));
    },
