define(["../fileUtils", "fs"], function(fileUtils, fs) {
  var 
    ...
    getReplacementFile= function(filename, encoding) {
      return (cached[filename]= cached[filename] || fs.readFileSynch(filename, encoding));
    },

    deReplacements= function(contents, replacement) {
    var encoding= "utf8";
  ...
  return function(resource, bc, asyncReturn) {
    ...
    var text= resource.getResultText && resource.getResultText() || resource.text;
    if (bc.replacements[resource.src]) {
      text= doReplacements(text, bc.replacements[resource.src]);
    }
